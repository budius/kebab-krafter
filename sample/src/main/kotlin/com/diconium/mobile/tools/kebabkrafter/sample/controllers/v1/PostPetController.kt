...
class PostPetController(private val createPet: suspend (Pet) -> Pet) : PostPet {
    override suspend fun CallScope.execute(body: PostPetRequest): Pet {
        val newPet = when (body.petType) {
            PostPetRequest.PetType.DOG -> Pet.Dog("", body.name, Color.BROWN, 5.3f, 8, true, "alpaca", body.dateOfBirth)
            PostPetRequest.PetType.CAT -> Pet.Cat("", body.name, Color.ORANGE_BROWN, 0f, body.dateOfBirth)
